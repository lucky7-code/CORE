...
class sparseKT(nn.Module):
    def __init__(self, n_question, n_pid, device, 
            d_model=256, n_blocks=2, dropout=0.1, d_ff=256, seq_len=200, 
            kq_same=1, final_fc_dim=512, final_fc_dim2=256, num_attn_heads=8, separate_qa=False, l2=1e-5, emb_type="qid_sparseattn", sparse_ratio=0.8, k_index=5, stride=1):
        super().__init__()
        """
        Input:
        ...
        if self.n_pid > 0:
            if emb_type.find("scalar") != -1:
                ...
                self.difficult_param = nn.Embedding(self.n_pid+1, embed_l, padding_idx=0) # 题目难度
    ...
    def forward(self, dcur):
        ...
        d_output, attn_weights = self.model(q_embed_data, qa_embed_data, self.device, emb_type, sparse_ratio, k_index)
        ...
        return loss, y, t, q + self.n_pid * t
...
class Architecture(nn.Module):
    ...
    def forward(self, q_embed_data, qa_embed_data, device, emb_type="qid", sparse_ratio=0.8, k_index=5):
        ...
        for block in self.blocks_2:
            x, attn_weights = block(mask=0, query=x, key=x, values=y, apply_pos=True, emb_type=emb_type, sparse_ratio=sparse_ratio, k_index=k_index, device=device) # True: +FFN+残差+laynorm 非第一层与0~t-1的的q的attention, 对应图中Knowledge Retriever
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, mask, query, key, values, device, apply_pos=True, emb_type="qid", sparse_ratio=0.8, k_index=5):
        """
        Input:
        ...
        if mask == 0:  # If 0, zero-padding is needed.
            ...
            query2,_ = self.masked_attn_head(
                query, key, values, mask=src_mask, zero_pad=True, emb_type=emb_type, sparse_ratio=sparse_ratio, k_index=k_index, device=device) # 只能看到之前的信息，当前的信息也看不到，此时会把第一行score全置0，表示第一道题看不到历史的interaction信息，第一题attn之后，对应value全0
        else:
            # Calls block.masked_attn_head.forward() method
            query2,_ = self.masked_attn_head(
                query, key, values, mask=src_mask, zero_pad=False, emb_type=emb_type, sparse_ratio=sparse_ratio, k_index=k_index, device=device)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask, zero_pad, device, emb_type="qid", sparse_ratio=0.8, k_index=5):
        ...
        scores, attn_weights = attention(q, k, v, self.d_k,
                           mask, self.dropout, zero_pad, device, emb_type, sparse_ratio=sparse_ratio, k_index=k_index)
...
def attention(q, k, v, d_k, mask, dropout, zero_pad, device, emb_type="qid", sparse_ratio=0.8, k_index=5):
    """
