...
class simpleKT(nn.Module):
    def __init__(self, n_question, n_pid, device, 
            ...
            kq_same=1, final_fc_dim=512, final_fc_dim2=256, num_attn_heads=8, separate_qa=True, l2=1e-5, emb_type="qid"):
        super().__init__()
        """
        Input:
        ...
        self.n_question = n_question # number of concepts
        self.dropout = dropout
        self.kq_same = kq_same
        self.n_pid = n_pid # number of problems
        self.l2 = l2
        ...
        self.model = Architecture(n_blocks=n_blocks, n_heads=num_attn_heads, dropout=dropout,
                                    d_model=d_model, d_ff=d_ff,  kq_same=self.kq_same, model_type=self.model_type, seq_len=seq_len, device=device)
...
class Architecture(nn.Module):
    def __init__(self,  n_blocks, d_model, d_ff, n_heads, dropout, kq_same, model_type, seq_len, device):
        super().__init__()
        """
    ...
    def forward(self, q_embed_data, qa_embed_data):
        ...
        for block in self.blocks_2:
            x = block(mask=0, query=x, key=x, values=y, device=self.device, apply_pos=True) # True: +FFN+残差+laynorm 非第一层与0~t-1的的q的attention, 对应图中Knowledge Retriever
...
class TransformerLayer(nn.Module):
    ...
    def forward(self, mask, query, key, values, device, apply_pos=True):
        """
        Input:
        ...
        if mask == 0:  # If 0, zero-padding is needed.
            ...
            query2 = self.masked_attn_head(
                query, key, values, mask=src_mask, zero_pad=True, device=device) # 只能看到之前的信息，当前的信息也看不到，此时会把第一行score全置0，表示第一道题看不到历史的interaction信息，第一题attn之后，对应value全0
        else:
            # Calls block.masked_attn_head.forward() method
            query2 = self.masked_attn_head(
                query, key, values, mask=src_mask, zero_pad=False, device=device)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask, zero_pad, device):
        ...
        scores = attention(q, k, v, self.d_k,
                           mask, self.dropout, zero_pad, device)
