...
import pandas as pd
from pykt.models.utils import transformer_FFN, get_clones, ut_mask, pos_encode
...
class SAINT(nn.Module):
    def __init__(self, num_q, num_c, seq_len, emb_size, num_attn_heads, dropout, n_blocks=1, emb_type="qid"):
        super().__init__()
    ...
    def forward(self, data):
        ...
        y = torch.masked_select(res[:, 1:], sm)
...
class Encoder_block(nn.Module):
    ...
    def __init__(self, dim_model, heads_en, total_ex, total_cat, seq_len, dropout, emb_path="", pretrain_dim=768):
        ...
        if total_cat > 0:
            self.emb_cat = nn.Embedding(total_cat+1, embedding_dim=dim_model,padding_idx=0)
    ...
    def multi_skills_embedding(self,c):
        concept_emb_sum = self.emb_cat(c + 1).sum(-2)
    ...
    def forward(self, in_ex, in_cat, in_pos, first_block=True):
        ...
        if first_block:
            ...
            if self.total_ex > 0:
                ...
                embs.append(in_ex)
            if self.total_cat > 0:
                in_cat = self.multi_skills_embedding(in_cat)
                embs.append(in_cat)
            out = embs[0]
